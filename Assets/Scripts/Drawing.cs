     ...
     public static class Drawing
     {
         ...
         public static void DrawCircle(Vector2 center, float radius, Color color, float width, int segments) {
             DrawCircle(center, radius, color, width, true, segments);
         }

         public static void DrawCircle(Vector2 center, float radius, Color color, float width, bool antiAlias, int segments) {
             float angleStep = 2*Mathf.PI/segments;
             float angleCurrent = 0;
             Vector2 a = new Vector2(center.x + radius,center.y);
             for (int i = 0; i < segments; i++)
             {
                angleCurrent+=angleStep;
                 Vector2 b = new Vector2(center.x + radius*Mathf.Cos(angleCurrent),center.y + radius*Mathf.Sin(angleCurrent));
                 Drawing.DrawLine(a, b, color, width, antiAlias);
                 a = b;
             }
         }
     
         public static void DrawCircle2(Vector2 center, float radius, Color color, float width, bool antiAlias, int segmentsPerQuarter) {
             float rh = radius / 2;
